...
@section('title', __('section_details_trans.add'))
@section('content')
    <form action="{{ route('section-details.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="form-group col-md-6">
                <label>{{ __('section_details_trans.title') }}</label>
                <input type="text" name="title" class="form-control" value="{{ old('title', $sectionDetail->title ?? '') }}">
        ...
        <div class="row">
            @foreach(['ar','en','fr','es','it','de'] as $locale)
                <div class="form-group col-md-6">
                    ...
                    <textarea name="content_{{ $locale }}" class="form-control" rows="3">{{ old('content_' . $locale, $sectionDetail->content[$locale] ?? '') }}</textarea>
        ...
        <div class="form-group">
            ...
            @if(!empty($sectionDetail->image))
                <img src="{{ asset($sectionDetail->image) }}" height="100"><br><br>
            @endif
        ...
        <div class="form-group">
            ...
            @if(!empty($sectionDetail->images))
                @foreach($sectionDetail->getImages() as $img)
                    <img src="{{ asset($img) }}" height="80" class="mr-2 mb-2">
                @endforeach
                <br><br>
            @endif
        ...
        <button type="submit" class="btn btn-success">{{ __('section_details_trans.submit') }}</button>
    </form>
@endsection
